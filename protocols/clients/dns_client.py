...
class Client:
    ...
    def negotiatedTransmit(self, data_to_transmit,config):

        byte_reader = 0
        check_total = False
        packet_number = 1

        print("[+] Sending DNS Data")
        ...
        while (byte_reader < len(data_to_transmit)):
            if not self.file_transfer:
                ...
                encoded_data = base64.b64encode(str(struct.pack('>I', packet_number)) + ".:|:." + data_to_transmit[
                                                                                                  byte_reader:byte_reader + self.length])
        ...
        if self.file_transfer is not False:
            while True:
                final_packet = sr1(IP(dst=final_destination) / UDP() / DNS(
                    id=15, opcode=0,
                    qd=[DNSQR(qname="ENDTHISFILETRANSMISSIONEGRESSASSESS" + self.file_transfer, qtype="TXT")], aa=1,
                    qr=0),
